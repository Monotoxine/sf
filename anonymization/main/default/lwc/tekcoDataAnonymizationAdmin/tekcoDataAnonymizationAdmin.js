...
export default class TekcoDataAnonymizationAdmin extends LightningElement {
    ...
    handleDeselectAllHistory()    { const v = new Set(this.fieldConfigsFiltered.map(c => c.configKey)); this.fieldConfigs = this.fieldConfigs.map(cfg => v.has(cfg.configKey) ? { ...cfg, deleteHistory: false } : cfg); }
    ...
    handleByIdDeselectAllHistory(){ const v = new Set(this.byIdFieldConfigsFiltered.map(c => c.configKey)); this.byIdFieldConfigs = this.byIdFieldConfigs.map(cfg => v.has(cfg.configKey) ? { ...cfg, deleteHistory: false } : cfg); }
